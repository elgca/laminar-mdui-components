...
object VersionHelper {
  ...
  def versionFmt(out: sbtdynver.GitDescribeOutput,
      dynverSonatypeSnapshots: Boolean): String = {
    if (out.isCleanAfterTag) {
      out.ref.dropPrefix
    } else {
      val dirtyPart = if (out.isDirty()) out.dirtySuffix.value else ""
      val snapshotPart =
        if (dynverSonatypeSnapshots && out.isSnapshot()) "-SNAPSHOT" else ""
      ...
      (if (out.ref.dropPrefix.matches(""".*-(M|RC)\d+$""")) {
         ...
         val mainBranchIsAncestor =
           Process("git merge-base --is-ancestor main HEAD")
             .run(ProcessLogger(_ => ()))
             .exitValue() == 0
         lazy val masterBranchIsAncestor =
           Process("git merge-base --is-ancestor master HEAD")
             .run(ProcessLogger(_ => ()))
             .exitValue() == 0
