...
abstract class WebComponent(tagName: String) extends CommonTypes { this: Self =>
  ...
  /**
   * Override this with JSImport-ed object of the component. The import must register the component's custom element in the DOM.
   */
  ...
  protected def tagWithControlledInput[
  ...
  def on[Ev <: dom.Event, V](ev: EventProcessor[Ev, V]): EventProcessor[Ev, V] =
    ev
  ...
  final def apply(mods: ComponentMod*): Element = {
    val el = tag()
    mods.foreach {
      case mod: Modifier[? >: Element] =>
        mod(el)
      case modFn: Function[? >: this.type, ? <: Modifier[Element]] =>
        modFn(this)(el)
    }
    el
  }
